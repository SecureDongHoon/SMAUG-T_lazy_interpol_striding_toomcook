...
#error "This implementation assumes CRYPTO_BYTES + PKSEED_BYTES to be 64"
    ...
    for(i = 0; i < MODULE_RANK; i++)
        evaluation_single((const uint16_t *)sk_tmp.vec[i].coeffs, sw[i]);
    genPubkey(&pk_tmp, sw, seed);
...
void indcpa_enc(uint8_t ctxt[CIPHERTEXT_BYTES],
    ...
    uint16_t sw[MODULE_RANK][7][64];
    for(i = 0; i < MODULE_RANK; i++)
        evaluation_single((const uint16_t *)r.vec[i].coeffs, sw[i]);
    computeC1(&(ctxt_tmp.c1), pk_tmp.A, sw);
    computeC2(&(ctxt_tmp.c2), mu, &pk_tmp.b, sw); // EDIT TiMER
...
void indcpa_dec(uint8_t delta[MSG_BYTES],
    ...
    for(i = 0; i < MODULE_RANK; i++)
        evaluation_single((const uint16_t *)sk_tmp.vec[i].coeffs, sw[i]);
    vec_vec_mult_add(&delta_temp, &c1_temp, sw, _16_LOG_P);
